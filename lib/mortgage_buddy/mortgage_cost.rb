...
module MortgageBuddy
  class MortgageCost
    include MortgageBuddy::Monthly
    include MortgageBuddy::SafeNum
    ...
    def monthly_payment_with_fees
      @monthly_payment_with_fees ||= calculate_monthly_payment(@loan_amount + total_fees, monthly_interest_rate,
                                                               @period)
    end
    ...
    def total_fees(negative_allowed = false)
      ...
      !negative_allowed && total_fees.negative? ? 0 : total_fees
    end
