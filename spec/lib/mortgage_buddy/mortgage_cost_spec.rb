...
describe MortgageBuddy::MortgageCost do
  let(:params) do
    {loan_amount:   100000,
     interest_rate: 6.0,
     period:        360,
  ...
  describe 'initialize' do
    ...
    context 'without loan_amount' do
      ...
      it 'raises error' do
        expect { subject }.to raise_error('[:loan_amount] required')
      end
    end
    ...
    context 'without interest_rate' do
      ...
      it 'raises error' do
        expect { subject }.to raise_error('[:interest_rate] required')
      end
    end
  ...
  describe 'check calculations ' do
    ...
    describe 'the monthly payment with fees should be the same as the monthly payment with APR as interest rate' do
      [{loan_amount: 300000, interest_rate: 6.5, period: 360, fees: 1200, points: 1.25},
       {loan_amount: 300000, interest_rate: 6.5, period: 360, fees: 0, points: 0},
       {loan_amount: 400000, interest_rate: 1.1, period: 180, fees: 1200, points: 1.25},
       {loan_amount: 300000, interest_rate: 6.5, period: 360, fees: 0, points: 7.25},
       {loan_amount: 300000, interest_rate: 6.5, period: 360, fees: 10000, points: 7.25}
      ].each do |test_hash|
  ...
  context 'net negative fees' do
    ...
    it 'calculates total fees' do
      expect(subject.send(:calculate_total_fees)).to eq(-10050.0)
    end

    it 'actual total fees' do
      expect(subject.total_fees(true)).to eq(-10050.0)
    end
